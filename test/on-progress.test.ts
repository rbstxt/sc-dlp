...
test('progress events received in correct order', async () => {
  const allEvents: any[][] = [];
  const startedEventTypes: string[] = [];
  const finishedEventTypes: string[] = [];
  const onProgress = (type: string, loaded: number, total: number) => {
    ...
    if (loaded === 0) {
      if (startedEventTypes[startedEventTypes.length - 1] !== type) {
